...
const CATEGORIES = [
  ...
  { slug: "grahapravesam", label: "Grahapravesam" },
  { slug: "bhoomi-poojai", label: "Bhoomi Poojai" },
  { slug: "punyahavachanam", label: "Punyahavachanam" },
  { slug: "ruthu-punyahavachanam", label: "Ruthu Punyahavachanam" },
  { slug: "ayush-homam", label: "AYUSH HOMAM" },
];
    ...
    return (
      <div>
        ...
        <p className="text-sm text-[#6B5A4E] mb-6">
          Choose a category to manage its Package Details.
        </p>
...
function PackageDetailsTable({
  ...
  async function loadItems() {
    ...
    setError(null);
    try {
      const res = await fetch(`${API_BASE}/consumer-packages/${category}`);
    ...
    try {
      if (editingId) {
        const res = await fetch(
          `${API_BASE}/consumer-packages/${category}/${editingId}`,
          {
            method: "PATCH",
            headers: { "Content-Type": "application/json" },
            credentials: "include",
  ...
  async function handleDelete(id: number) {
    ...
    try {
      const res = await fetch(
        `${API_BASE}/consumer-packages/${category}/${id}`,
  ...
  async function handleDeleteAll() {
    ...
    try {
      const res = await fetch(
        `${API_BASE}/consumer-packages/${category}/all`,
        { method: "DELETE", credentials: "include" },
      );
      if (!res.ok) throw new Error();
